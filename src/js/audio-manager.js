// Standalone Audio Manager - Works without module imports
// This provides the audio functionality for both traditional pages and SPA
...
(function() {
    ...
    class AudioManagerService {
        constructor() {
            // UI/FX sounds
            this.selectSound = new Audio(ASSET_BASE + 'assets/sounds/select.wav');
            // Mythical theme track (muted by default; only plays after click)
            this.themeMusic = new Audio(ASSET_BASE + 'assets/sounds/mythic-theme.mp3');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!document.querySelector('.music-toggle')) {
            ...
            musicToggle.addEventListener('click', () => {
                musicToggle.innerHTML = window.AudioManager.toggleMusic();
            });
            
            musicToggle.addEventListener('mouseenter', () => {
                musicToggle.style.transform = 'scale(1.1)';
                musicToggle.style.boxShadow = '0 6px 12px rgba(0, 0, 0, 0.4)';
            });
            
            musicToggle.addEventListener('mouseleave', () => {
                musicToggle.style.transform = 'scale(1)';
                musicToggle.style.boxShadow = '0 4px 8px rgba(0, 0, 0, 0.3)';
            });
        }
    });
})();